...
class InvestInBusiness extends StatefulWidget {
  static String routeName = 'investInBusiness';

  @override
  _InvestInBusinessState createState() => _InvestInBusinessState();
}

class _InvestInBusinessState extends State<InvestInBusiness> {
  Future balance;
  ...
  void buyEquityMethod() {
    ...
    buyEquity(token, selectedBusiness['businessAddress'],
          ...
          child: Column(
            children: [
              ...
              Container(
                ...
                child: Image.asset(
                  'assets/images/InvestInBusiness.png',
                  width: MediaQuery.of(context).size.width - 32,
                  height: 300,
                ),
              ),
                ...
                child: Column(
                  children: [
                    Form(
                      key: _formKey,
                      child: TextFormField(
                        decoration: InputDecoration(labelText: 'VPM amount'),
                        controller: amountController,
                        keyboardType: TextInputType.number,
                        onChanged: checkLength(),
                        validator: RequiredValidator(
                            errorText: 'Please enter a valid VPM amount'),
                      ),
                    ),
                    Padding(
                      ...
                      child: ElevatedButton(
                        child: Row(
                          ...
                          children: [
                            Padding(
                              ...
                              child: Text(
                                'Invest in ' + selectedBusiness['businessName'],
                                style: TextStyle(
                                  fontSize: 20,
                                ),
                              ),
                        ...
                        onPressed: () {
                          if (_formKey.currentState.validate()) {
                            ...
                            if (int.parse(amountController.text) >
                                int.parse(selectedBusiness['target'])) {
                              return ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  backgroundColor:
                                      Theme.of(context).primaryColor,
                            ...
                            if (int.parse(selectedBusiness['target']) -
                                    int.parse(selectedBusiness['raised']) <
                                int.parse(amountController.text)) {
                              return ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  backgroundColor:
                        ...
                        ? Text(
                            'You will own ' +
                                (int.parse(amountController.text) *
                                        ...
                                        (int.parse(selectedBusiness['target']) *
                                            100 /
                                            int.parse(selectedBusiness['equity'])))
                                    .toStringAsFixed(2) +
                                "% of " +
...
Future<dynamic> getUserBalance(String token) async {
  ...
  var request = http.Request(
      'GET', Uri.parse('http://192.168.203.76:8080/api/getUsersAccountBalance'));
...
Future<dynamic> buyEquity(String token, String address, int amount) async {
  ...
  var request =
      http.Request('POST', Uri.parse('http://192.168.203.76:8080/api/buyEquity'));
  request.body = json.encode({"business_address": address, "amount": amount});
  request.headers.addAll(headers);
