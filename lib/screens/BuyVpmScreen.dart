...
class BuyVpmScreen extends StatefulWidget {
  static const routeName = '/buyVpmScreen';

  @override
  _BuyVpmScreenState createState() => _BuyVpmScreenState();
}

class _BuyVpmScreenState extends State<BuyVpmScreen> {
  String token;
  ...
  void onGooglePayResult(paymentResult) {
    ...
    buyVPM(token, int.parse(amountController.text)).then((data) {
      if (data['result'] == true) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          ...
          padding: EdgeInsets.all(20),
          content: Text(
            "There was a problem buying VPM",
            textAlign: TextAlign.center,
          ),
        ));
      }
            ...
            children: [
              ...
              isFormValid
                  ...
                  : Container(
                      ...
                      child: Text(
                        'Please enter the amount of VPM you want to buy above.',
                        textAlign: TextAlign.center,
                      ),
                    ),
            ],
...
Future<dynamic> buyVPM(String token, int amount) async {
  var headers = {
    'x-api-key':
  ...
  var request = http.Request(
      'POST', Uri.parse('http://192.168.203.76:8080/api/transferVPMToUser'));
